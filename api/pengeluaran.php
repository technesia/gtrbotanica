...
try {
    if ($method === 'GET') {
        ...
        $stmt = $pdo->prepare('SELECT id, tanggal, kategori, deskripsi, jumlah, petugas FROM pengeluaran ORDER BY tanggal DESC, id DESC LIMIT :limit');
    ...
    if ($method === 'POST') {
        ...
        $stmt = $pdo->prepare('INSERT INTO pengeluaran (tanggal, kategori, deskripsi, jumlah, petugas, keterangan) VALUES (:tanggal, :kategori, :deskripsi, :jumlah, :petugas, :keterangan)');
        ...
        $stmt->bindValue(':keterangan', $deskripsi); // fallback isi keterangan sama dengan deskripsi
        $stmt->execute();
